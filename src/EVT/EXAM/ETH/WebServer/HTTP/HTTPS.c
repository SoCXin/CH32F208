...
uint8_t basicbuf[BASIC_CFG_LEN], portbuf[PORT_CFG_LEN], loginbuf[LOGIN_CFG_LEN];
...
uint8_t Basic_Default[BASIC_CFG_LEN] = {
0x57, 0xAB,
01, 02, 03, 04, 05, 06, 192, 168, 1, 10, 255, 255, 255, 0, 192, 168, 1, 1};

/*WCHNET login default parameters, user name and password*/
uint8_t Login_Default[LOGIN_CFG_LEN] = {
0x57, 0xAB,
'a', 'd', 'm', 'i', 'n', 0, 0, 0, 0, 0, '1', '2', '3', 0, 0, 0, 0, 0, 0, 0 };

/*WCHNET default port configuration*/
uint8_t Port_Default[PORT_CFG_LEN] = {
0x57, 0xAB,
MODE_TCPCLIENT, 1000 / 256, 1000 % 256, 192, 168, 1, 100, 1000 / 256, 1000 % 256 };

uint8_t *name;                                               //The name of the web page requested by HTTP
uint8_t socket;                                              //socket id
uint8_t httpweb[200];                                        //The array is used to store the HTTP response message
...
const char Html_main[] = {
    ...
    "</a>\r\n"
    "</li>\r\n"
    "<!--The fourth line of the directory -->\r\n"
    "<li id=\"4\" >\r\n"
...
{
    if (strstr(buf, ".html") || (strlen((char *)name) == 1)) //html type
        *type = PTYPE_HTML;
 ...
 * @return  none
 */
void MakeHttpResponse(uint8_t *buf, char type)
{
    char *head = 0;
 ...
 * @return  none
 */
void Refresh_Html(const char *html, Parameter *buf, uint8_t paranum)
{
    const char *p1;
    char *p2, *q;
    char para[10];
 ...
 * @return  FLASH_Status
 */
FLASH_Status WEB_WRITE(u32 StartAddr, uint8_t *Buffer, u32 Length) {
 ...
 * @return  none
 */
void WEB_READ(u32 StartAddr, uint8_t *Buffer, u32 Length) {
    u32 address = StartAddr;
    u32 *p_buff = (u32 *) Buffer;
...
void Init_Para_Tab(void)
{
    ...
    memset(s, 0, 20);
    sprintf((char *)s, "%d.%d.%d.%d.%d.%d", Basic_CfgBuf->mac[0], Basic_CfgBuf->mac[1],
            Basic_CfgBuf->mac[2], Basic_CfgBuf->mac[3], Basic_CfgBuf->mac[4],
            Basic_CfgBuf->mac[5]);
    strcpy(Para_Basic[0].value, (char *)s);
    printf("__AMAC=%s\n", Para_Basic[0].value);

    Para_Basic[1].para = "__ASIP";
    memset(s, 0, 20);
    sprintf((char *)s, "%d.%d.%d.%d", Basic_CfgBuf->ip[0], Basic_CfgBuf->ip[1],
            Basic_CfgBuf->ip[2], Basic_CfgBuf->ip[3]);
    strcpy(Para_Basic[1].value, (char *)s);
    printf("__ASIP=%s\n", Para_Basic[1].value);

    Para_Basic[2].para = "__AMSK";
    memset(s, 0, 20);
    sprintf((char *)s, "%d.%d.%d.%d", Basic_CfgBuf->mask[0], Basic_CfgBuf->mask[1],
            Basic_CfgBuf->mask[2], Basic_CfgBuf->mask[3]);
    strcpy(Para_Basic[2].value, (char *)s);
    printf("__AMSK=%s\n", Para_Basic[2].value);

    Para_Basic[3].para = "__AGAT";
    memset(s, 0, 20);
    sprintf((char *)s, "%d.%d.%d.%d", Basic_CfgBuf->gateway[0],
            Basic_CfgBuf->gateway[1], Basic_CfgBuf->gateway[2],
            Basic_CfgBuf->gateway[3]);
    strcpy(Para_Basic[3].value, (char *)s);
    printf("__AGAT=%s\n", Para_Basic[3].value);

    Para_Port[0].para = "__AMOD";
    memset(s, 0, 20);
    sprintf((char *)s, "%d", Port_CfgBuf->mode);
    strcpy(Para_Port[0].value, (char *)s);

    Para_Port[1].para = "__ASPT";
    memset(s, 0, 20);
    sprintf((char *)s, "%d", Port_CfgBuf->src_port[0] * 256 + Port_CfgBuf->src_port[1]);
    strcpy(Para_Port[1].value, (char *)s);

    Para_Port[2].para = "__ADIP";
    memset(s, 0, 20);
    sprintf((char *)s, "%d.%d.%d.%d", Port_CfgBuf->des_ip[0], Port_CfgBuf->des_ip[1],
            Port_CfgBuf->des_ip[2], Port_CfgBuf->des_ip[3]);
    strcpy(Para_Port[2].value, (char *)s);

    Para_Port[3].para = "__ADPT";
    memset(s, 0, 20);
    sprintf((char *)s, "%d", Port_CfgBuf->des_port[0] * 256 + Port_CfgBuf->des_port[1]);
    strcpy(Para_Port[3].value, (char *)s);
...
void Web_Server(void)
{
    ...
    if(DealDataFlag){
        DealDataFlag = 0;
        ParseHttpRequest(http_request, (char *)RecvBuffer);
        switch (http_request->METHOD)
        {
            ...
            case METHOD_POST:                                       //'post' request
                name = (uint8_t*)GetURLName(http_request->URL);
                //printf("URL name: %s\n",name);
                ParseURLType(&http_request->TYPE, (char *)name);
                ...
                if (strstr((char *)name, "main") != NULL) {                 //Request the "main" page
                    len = strlen(Html_main);
                    copy_flash(Html_main, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "success") != NULL) {          //Request "success" page
            ...
            case METHOD_GET:                                        //'get' request
                ...
                else if(strstr((char *)name, "main") != NULL) {             //Request to get the "main" web page
                    len = strlen(Html_main);
                    copy_flash(Html_main, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "basic") != NULL) {            //Request to get the "basic" web page
                    Refresh_Html(Html_basic, Para_Basic, 4);
                    len = strlen(HtmlBuffer);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "port") != NULL) {             //Request to get the "port" page
                    Refresh_Html(Html_port, Para_Port, 4);
                    len = strlen(HtmlBuffer);
                    WCHNET_SocketSend(socket,(uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "user") != NULL) {             //Request to get the "user" web page
                    Refresh_Html(Html_user, Para_Login, 2);
                    len = strlen(HtmlBuffer);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "about") != NULL) {            //Request to get the "about" page
                    len = strlen(Html_about);
                    copy_flash(Html_about, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "logo") != NULL) {             //Request for "logo" image
                    len = sizeof(Html_logo);
                    copy_flash(Html_logo, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "png1") != NULL) {             //Request to get "png1" image
                    len = sizeof(Html_png1);
                    copy_flash(Html_png1, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "png2") != NULL) {             //Request to get "png2" image
                    len = sizeof(Html_png2);
                    copy_flash(Html_png2, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "png3") != NULL) {             //Request to get "png3" image
                    len = sizeof(Html_png3);
                    copy_flash(Html_png3, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);    //Request to get "png4" image
                }
                else if(strstr((char *)name, "png4") != NULL) {
                    len = sizeof(Html_png4);
                    copy_flash(Html_png4, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "weixin") != NULL) {           //Request for "weixin" image
                    len = sizeof(Html_weixin);
                    copy_flash(Html_weixin, len);
                    WCHNET_SocketSend(socket, (uint8_t *)HtmlBuffer, &len);
                }
                else if(strstr((char *)name, "style") != NULL) {            //Request to get the "style" css stylesheet file
