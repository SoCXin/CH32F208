 ...
 *@Note
   ...
   b-When the main frequency exceeds 100MHz, attention should be paid when operating the FLASH:
			When performing non-zero waiting area FLASH and zero waiting area FLASH, reading and writing 
			user words, and reading manufacturer configuration words and Boot area, the following operations
			are required.
			First divide the frequency of HCLK by 2, and resume after the FLASH operation is completed to 
			ensure that the frequency of the FLASH operation is lower than 100Mhz.
...
{
    printf("FLASH Test\n");
/*/*在主频超过100MHz时，操作FLASH时需注意：将HCLK进行二分频，会导致HCLK下相关外设时钟二分频，使用时需注意*/
